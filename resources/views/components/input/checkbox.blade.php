...
>
    ...
    >
        <x-tabler-icons.check x-bind:class="{'hidden': !checked, 'block': checked}" class="w-4 h-4 text-white" />
    </div>
    @if($label)
        <label class="text-xs cursor-pointer">{{ $label }}</label>
    @endif
    <input x-ref="input" @change="checked = $el.checked" type="checkbox" {{ $attributes }} style="display: none;" />
</button>
