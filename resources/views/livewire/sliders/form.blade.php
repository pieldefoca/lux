<form wire:submit.prevent="save" class="max-w-4xl mx-auto space-y-10">
    ...
    <div>
        <x-lux::card title="Diapositivas" class="p-4">
            ...
            @else
                <div drag-root="reorderSlides" class="space-y-5 mt-4">
                    @foreach($slider->slides()->orderBy('sort')->get() as $slide)
                        <div drag-item="{{ $slide->id }}" draggable="true" class="flex items-center space-x-4">
                            <x-lux::tabler-icons.grip-horizontal class="opacity-40 hover:opacity-60 cursor-grab" />

                            <div class="flex-grow flex items-end space-x-4">
                                <div>
